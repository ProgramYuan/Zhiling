...
此版本可以将u_char *audiobuf里面的数据拷贝到u_char audiobuf[]数组空间里
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		char* argvs[] = { "arecord", "-Dplughw:0,0","-f","S16_LE","-Vmono","stdout"};

		funcMain(sizeof(argvs) / sizeof(argvs[0]), argvs);
...
int funcMain(int argc, char *argv[]) {
	...
	static const struct option long_options[] = { { "help", 0, 0, 'h' }, {
			...
			"device", 1, 0, 'D'
		}, { "quiet", 0, 0, 'q' }, { "file-type", 1, 0,
			't'
		}, { "channels", 1, 0, 'c' }, { "format", 1, 0, 'f' }, {
			"rate", 1, 0, 'r'
		}, { "duration", 1, 0, 'd' },
		{ "mmap", 0, 0, 'M' }, { "nonblock", 0, 0, 'N' }, { "period-time",
			1, 0, 'F'
			...
			'v'
		}, { "vumeter", 1, 0, 'V' }, { "separate-channels", 0,
			0, 'I'
		}, { "playback", 0, 0, 'P' },
		{ "capture", 0, 0, 'C' }, { "disable-resample", 0, 0,
			OPT_DISABLE_RESAMPLE
		}, { "disable-channels", 0, 0,
			OPT_DISABLE_CHANNELS
		}, { "disable-format", 0, 0,
			OPT_DISABLE_FORMAT
		}, { "disable-softvol", 0, 0,
		...
		{ "test-nowait", 0, 0, OPT_TEST_NOWAIT }, { "max-file-time", 1, 0,
			OPT_MAX_FILE_TIME
		}, { "process-id-file", 1, 0,
			OPT_PROCESS_ID_FILE
		}, { "use-strftime", 0, 0,
		...
		case 'f':
			if (strcasecmp(optarg, "cd") == 0 || strcasecmp(optarg, "cdr") == 0) {
				if (strcasecmp(optarg, "cdr") == 0)
					...
					rhwparams.format
					    = file_type == FORMAT_AU ? SND_PCM_FORMAT_S16_BE
	...
	return 1; //返回1给上面的调用程序，上面调用程序判断拾音结束
__end: snd_output_close(log);
...
static void print_vu_meter_mono(int perc, int maxperc) {
	...
	if (perc >= 15) //speak
	{
		...
		//init silcount 1s
		silcount = 1 * snd_pcm_format_size(hwparams.format, hwparams.rate
		                                   * hwparams.channels);
		silflag = 1;
	} else {
...
static void begin_wave(int fd, size_t cnt) {
...
#else
	tmp2 = hwparams.channels * snd_pcm_format_physical_width(hwparams.format)
	       / 8;
	f.byte_p_spl = LE_SHORT(tmp2);
	...
	if (write(fd, &h, sizeof(WaveHeader)) != sizeof(WaveHeader) || write(fd,
	        &cf, sizeof(WaveChunkHeader)) != sizeof(WaveChunkHeader) || write(
	            fd, &f, sizeof(WaveFmtBody)) != sizeof(WaveFmtBody) || write(fd,
	                    &cd, sizeof(WaveChunkHeader)) != sizeof(WaveChunkHeader)) {
		error(_("write error"));
		prg_exit(EXIT_FAILURE);
	}
}
...
static void end_au(int fd) { /* only close output */
	AuHeader ah;
	off64_t length_seek;
...
static void header(int rtype, char *name) {
	if (!quiet_mode) {
		...
		fprintf(stderr, "%s %s '%s' : ",
		        (stream == SND_PCM_STREAM_PLAYBACK) ? _("Playing")
		        : _("Recording"),
		        gettext(fmt_rec_table[rtype].what), name);
...
static void capture(char *orig_name) {
	int tostdout = 0; /* 布尔值描述输出流 */
	int filecount = 0; /* 写入的文件数 */
    ...
    //测试
    int ssr=0;
    struct msg_audio audio_data[40];
    int a_size=0;
	int fds;
	do {
		fds=safe_open("121.pcm");
		/* 打开文件写入 */
		if (!tostdout) {
		...
		while (rest > 0 && recycle_capture_file == 0 && !in_aborting) {
			...
			//printf("%s",audio_data[ssr].AudioBuf);
			// printf("a_size:%d\n",a_size);
			// printf("f:%d\n",f);
			// printf("c:%d\n",c);
            ssr++;
	...
	if(ssr>8)
	{
		...
		{
			printf("%d\n",a_size);
			u_char vBuffer[a_size];
			memcpy(vBuffer,audio_data[sk].AudioBuf,a_size);
			if (write(fds, vBuffer, a_size) != a_size) {
				prg_exit(EXIT_FAILURE);
			}
			// if (write(fds, audio_data[sk].AudioBuf, a_size) != a_size) {
			// 	prg_exit(EXIT_FAILURE);
			// }
		}
	}
	...
	// 		shared->AudioBuf=(u_char *)malloc(a_size);
	// 		memcpy(shared->AudioBuf,audio_data[sk].AudioBuf,a_size);
	// 		printf("%s\n",shared->AudioBuf);
	// 		//写完数据，设置written使共享内存段可读  
	// 		shared->written = 1;  
	// 		// int sask=0;
	// 		// for(sask=0;sask<ssr;sask++)
	// 		// {
	// 		// 	// some_data.AudioBuffer=(u_char *)malloc(a_size);
	// 		// 	// memcpy(some_data.AudioBuffer,audio_data[sask].AudioBuf,a_size);
