...
#include <sys/ipc.h>
#include <sys/msg.h>
#include "../inc/sound/aconfig.h"
#include "../inc/sound/gettext.h"
#include "../inc/sound/formats.h"
#include "../inc/sound/version.h"
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		char* argvs[] = { "arecord", "-Dplughw:0,0","-f","S16_LE","-Vmono", wavFileName };

		funcMain(sizeof(argvs) / sizeof(argvs[0]), argvs);

		int _fileSize=get_file_size(wavFileName);
		...
		if (_fileSize > 37000) {
			...
			strcpy(some_data.some_text,wavFileName);
			...
			//Delete the file of wave that size less then 13K
			if(remove(wavFileName)==0)
			{
				#ifdef Conditional
					printf("%s已删除\n",wavFileName);
				#else
...
static void capturev_go(int* fds, unsigned int channels, off64_t count,
                        int rtype, char **names) {
	size_t c;
	ssize_t r;
	unsigned int channel;
	size_t vsize;
...
__end: for (channel = 0; channel < channels; ++channel) {
		if (fds[channel] >= 0)
			close(fds[channel]);
		if (alloced)
			free(names[channel]);
