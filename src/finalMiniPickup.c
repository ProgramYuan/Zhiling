...
u_char tmpAudioBuf[4000]; //临时空间
int criticalPoint = 20;
int v_volume_i = 0;
long vvolume = 0;  //用来存储环境音
...
static void print_vu_meter_mono(int perc, int maxperc) {
	...
	{
		//criticalPoint = (vvolume/50) > 20 ? ((vvolume/50) > 70 ? 70 : (vvolume/50)+5) : 20 ;
		criticalPoint = (vvolume/50) > 70 ? 70 : (vvolume/50)+5;
		vvolume = 0;
		v_volume_i = 0;
	}
...
int main(int argc, char *argv[]) {
	...
	off64_t count, rest; 
	int _silcount=(1 * snd_pcm_format_size(hwparams.format, hwparams.rate * hwparams.channels))/3;
	
	while(1)
	{
		if((access("online",F_OK))!=-1)   
		{   
			creat("shOnline",0755);
			if(unlink("online")<0)
			{
